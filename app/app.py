    ...
    if request.method == 'POST':
        ...
        cursor.execute("""
            ...
            SET numero = %s,
                ...
                sucursal_id = %s
            WHERE id = %s
        """, (numero, tipo, precio, sucursal_id, id))
        mysqldb.connection.commit()
...
def facturas():
    cursor = mysqldb.connection.cursor()
    cursor.execute('''
        SELECT facturas.id, facturas.fecha_emision, facturas.total, reservas.id AS reserva_id, clientes.nombre AS cliente_nombre
        FROM facturas
        JOIN reservas ON facturas.reserva_id = reservas.id
        JOIN clientes ON reservas.cliente_id = clientes.id
    ''')
    facturas = cursor.fetchall()
    return render_template('facturas/facturas.html', facturas=facturas)

@app.route('/add_factura', methods=['POST']) # Ruta para agregar facturas
def add_factura():
    if request.method == 'POST':
        reserva_id = request.form['reserva_id']
        ...
        total = request.form['total']
        cursor = mysqldb.connection.cursor()
        cursor.execute('INSERT INTO facturas (reserva_id, fecha_emision, total) VALUES (%s, %s, %s)', (reserva_id, fecha_emision, total))
        mysqldb.connection.commit()
        flash('Factura agregada exitosamente!')
        return redirect(url_for('facturas'))

@app.route('/edit_factura/<id>', methods=['POST', 'GET']) # Ruta para editar facturas
def get_factura(id):
    cursor = mysqldb.connection.cursor()
    ...
    cursor.execute('SELECT id FROM reservas')
    reservas = cursor.fetchall()
    return render_template('facturas/edit-factura.html', factura=factura, reservas=reservas)

@app.route('/update_factura/<id>', methods=['POST']) # Ruta para actualizar facturas
    ...
    if request.method == 'POST':
        ...
        cursor.execute('''
            ...
            SET reserva_id = %s,
                ...
                total = %s
            WHERE id = %s
        ''', (reserva_id, fecha_emision, total, id))
        mysqldb.connection.commit()
        flash('Factura actualizada exitosamente!')
        return redirect(url_for('facturas'))

@app.route('/delete_factura/<string:id>', methods=['POST']) # Ruta para eliminar facturas
def delete_factura(id):
    try:
        ...
        flash(f'Error al eliminar factura: {str(e)}')
    return redirect(url_for('facturas'))

@app.route('/historial_reservas') # Ruta para mostrar historial de reservas
def historial_reservas():
    cursor = mysqldb.connection.cursor()
    cursor.execute('''
        SELECT historial_reservas.id, historial_reservas.accion, historial_reservas.fecha, reservas.id AS reserva_id, clientes.nombre AS cliente_nombre
        FROM historial_reservas
        JOIN reservas ON historial_reservas.reserva_id = reservas.id
        JOIN clientes ON reservas.cliente_id = clientes.id
    ''')
    historial = cursor.fetchall()
    return render_template('historial_reservas/historial_reservas.html', historial=historial)

@app.route('/add_historial_reserva', methods=['POST']) # Ruta para agregar historial de reservas
def add_historial_reserva():
    if request.method == 'POST':
        reserva_id = request.form['reserva_id']
        accion = request.form['accion']
        cursor = mysqldb.connection.cursor()
        cursor.execute('INSERT INTO historial_reservas (reserva_id, accion) VALUES (%s, %s)', (reserva_id, accion))
        mysqldb.connection.commit()
        flash('Historial de reserva agregado exitosamente!')
        return redirect(url_for('historial_reservas'))

@app.route('/edit_historial_reserva/<id>', methods=['POST', 'GET']) # Ruta para editar historial de reservas
def get_historial_reserva(id):
    cursor = mysqldb.connection.cursor()
    cursor.execute('SELECT * FROM historial_reservas WHERE id = %s', (id,))
    historial = cursor.fetchone()
    cursor.execute('SELECT id FROM reservas')
    reservas = cursor.fetchall()
    return render_template('historial_reservas/edit-historial_reserva.html', historial=historial, reservas=reservas)

@app.route('/update_historial_reserva/<id>', methods=['POST']) # Ruta para actualizar historial de reservas
def update_historial_reserva(id):
    if request.method == 'POST':
        reserva_id = request.form['reserva_id']
        accion = request.form['accion']
        cursor = mysqldb.connection.cursor()
        cursor.execute('''
            ...
            SET reserva_id = %s,
                accion = %s
            WHERE id = %s
        ''', (reserva_id, accion, id))
        mysqldb.connection.commit()
        flash('Historial de reserva actualizado exitosamente!')
        return redirect(url_for('historial_reservas'))

@app.route('/delete_historial_reserva/<string:id>', methods=['POST']) # Ruta para eliminar historial de reservas
def delete_historial_reserva(id):
    try:
        cursor = mysqldb.connection.cursor()
        cursor.execute('DELETE FROM historial_reservas WHERE id = %s', (id,))
        mysqldb.connection.commit()
        flash('Historial de reserva eliminado exitosamente!')
    except Exception as e:
        flash(f'Error al eliminar historial de reserva: {str(e)}')
    return redirect(url_for('historial_reservas'))
